...
//     let line: Segment = LineSegment::new([0.0, 0.0], [1.0, 0.0],
// DEFAULT_EPSILON, DEFAULT_MAX_ULPS)         .unwrap()
//         .into();
...
// /**
// This is a regression test due to a bug which was found in the WindingExplorer
// app  */
// #[test]
...
//     let config = Viewport::from_bounded_entity(&shape,
// SideLength::Long(500));     return (shape, style, config);
// }
...
// |cr| {                 // Set the background to white
//                 cr.set_source_rgb(1.0, 1.0, 1.0);
//                 cr.paint()?;
//                 shape.draw(&style, cr)?;
//                 return Ok(());
...
//         view.compare_or_create(
//
// std::path::Path::new("tests/img/
// triangle_top_right_text_unscaled_offset_scaled.png"),             |cr| {
//                 // Set the background to white
//                 cr.set_source_rgb(1.0, 1.0, 1.0);
//                 cr.paint()?;
//                 shape.draw(&style, cr)?;
...
//     let view =
//         Viewport::from_bounding_box(&BoundingBox::from(&segment_chain),
// SideLength::Long(500));
...
//     let vertices = vec![[10f64, 0.0], [10f64, 10f64], [-10f64, 10f64],
// [-10f64, 0.0]];     let fillets = vec![1f64, 1f64, 1f64, 1f64];
//     let contour = Contour::new(
//         SegmentChain::from_fillets(vertices, fillets, true, DEFAULT_EPSILON,
// DEFAULT_MAX_ULPS)             .unwrap(),
//     );
...
//     let vertices = vec![[10e3, 0.0], [10e3, 10e3], [-10e3, 10e3], [-10e3,
// 0.0]];     let fillets = vec![1e3, 1e3, 1e3, 1e3];
//     let contour = Contour::new(
//         SegmentChain::from_fillets(vertices, fillets, true, DEFAULT_EPSILON,
// DEFAULT_MAX_ULPS)             .unwrap(),
//     );
...
//     // Now with another scale
//     let vertices = vec![[10e-3, 0.0], [10e-3, 10e-3], [-10e-3, 10e-3],
// [-10e-3, 0.0]];     let fillets = vec![1e-3, 1e-3, 1e-3, 1e-3];
//     let contour =
//         SegmentChain::from_fillets(vertices, fillets, true, DEFAULT_EPSILON,
// DEFAULT_MAX_ULPS)             .unwrap()
//             .into();
...
fn test_line_style() {
    ...
    view.write_to_file("images/line_styles.svg", create_fn)
        .unwrap();
