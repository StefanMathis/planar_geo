...
which can be used to draw the intersection as a cross. The properties of
...
[`Composite`] ([`Polysegment`], [`Contour`](crate::contour::Contour) or
...
let chain =
    Polysegment::from_points(&[[-1.0, 1.0], [-0.5, 0.5], [1.5, 0.5], [2.0, 1.0]]);
...
fn draw_segment_of_chain(
    ...
    if let Some(segment) = polysegment.get(segment_idx.0) {
...
fn draw_segment_of_shape(
    ...
    if let Some(contour) = shape.contours().get(shape_idx.contour_idx) {
        draw_segment_of_chain(contour.polysegment(), style, shape_idx.segment_idx, context)?;
    }
    return Ok(());
}
...
impl Intersection<ShapeIdx, ShapeIdx> {
    ...
    [`Polysegment`], [`Contour`](crate::contour::Contour) or [`Shape`] can also
    be drawn onto the canvas if the corresponding `(&Composite, &Style)` tuple
    is provided. See the [module documentation](crate::draw::intersection)
    for an example.
     */
    pub fn draw(
        &self,
...
impl Intersection<ShapeIdx, SegmentIdx> {
    /**
    Draws the intersection between a [`Shape`] and a [`Polysegment`] onto the
    ...
    pub fn draw(
        ...
        style: &IntersectionStyle,
        left: Option<(&Shape, &Style)>,
        right: Option<(&Polysegment, &Style)>,
        context: &cairo::Context,
    ) -> Result<(), cairo::Error> {
        if let Some(s) = left {
            draw_segment_of_shape(s.0, s.1, self.left, context)?;
...
impl Intersection<SegmentIdx, ShapeIdx> {
    /**
    Draws the intersection between a [`Polysegment`] and a [`Shape`] onto the
    given [`cairo::Context`] using the given [`IntersectionStyle`].
...
impl Intersection<SegmentIdx, SegmentIdx> {
    /**
    Draws the intersection between two [`Polysegment`]s onto the given
    [`cairo::Context`] using the given [`IntersectionStyle`].
...
impl Intersection<SegmentIdx, ()> {
    /**
    Draws the intersection between a [`Polysegment`] and a
    [`Primitive`](crate::primitive::Primitive) onto the given [`cairo::Context`]
    using the given [`IntersectionStyle`].
...
impl Intersection<(), SegmentIdx> {
    /**
    Draws the intersection between a [`Primitive`](crate::primitive::Primitive)
    and a [`Polysegment`] onto the given [`cairo::Context`] using the given
    [`IntersectionStyle`].
