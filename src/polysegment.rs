...
use crate::{
    DEFAULT_EPSILON, DEFAULT_MAX_RELATIVE, ToleranceContext, Transformation, composite::*,
};
...
impl Polysegment {
    ...
    */
    ...
    /**
    Check if the two intersection segments are just connected by glue segments.
    In that case, reject any intersection which is the stop point of the left
    segment and the start point of the right segment.
     */
    fn filter_intersections(&self, i: &Intersection, epsilon: f64, max_relative: f64) -> bool {
        ...
        ) && ((i.left.segment_idx + 1)..i.right.segment_idx).all(|i| {
            let s = &self[i];
            relative_eq!(
                s.start(),
                s.stop(),
...
impl<'c> Composite for ToleranceContext<'c, Polysegment> {
...
impl CompositeWithTol for Polysegment {
    ...
    fn covers_point_tol(
        ...
        match self
            .intersections_point_tol(point, epsilon, max_relative)
            .next()
        {
    ...
    fn intersections_polysegment_tol<'a>(
        ...
        let self_intersection = std::ptr::eq(self, other);
    ...
    fn intersections_shape_tol<'a>(
        ...
        shape
            .intersections_polysegment_tol(self, epsilon, max_relative)
    ...
    fn intersections_shape_par_tol<'a>(
        ...
        shape
            .intersections_polysegment_par_tol(self, epsilon, max_relative)
            .map(Intersection::switch)
    }
...
impl<'c> ToleranceContext<'c, Polysegment> {
    pub fn intersection_cut(&self, other: &Polysegment) -> Vec<Polysegment> {
            ...
            if start != stop {
                match segment {
                    ...
                    Segment::ArcSegment(a) => {
                        // If start and stop are (almost) identical, don't create an arc segment.
                        ...
                        // arc segment "a".
                        if relative_eq!(start, stop, epsilon = epsilon, max_relative = max_relative)
                        {
                            return None;
                        }
                        return ArcSegment::from_start_stop_center_radius(
                            start,
                            stop,
                            center,
                            a.radius(),
        ...
        for segment_self in self.inner.0.iter() {
            ...
            for segment_other in other.0.iter() {
                let intersections_segment_other = segment_self.intersections_segment_tol(
                    segment_other,
                    self.epsilon,
                    self.max_relative,
                );
    ...
    I: Iterator<Item = [f64; 2]>,
{
    ...
    // self_intersections the last iterator element
                        ...
                        Some(c) => {
                            if relative_eq!(
                                ...
                                if let Ok(new_combined) = LineSegment::new(c.start(), sl.stop()) {
                                    if new_combined.covers_line_segment_tol(
                                        line_segment,
                                        epsilon,
                                        max_relative,
                                    ) {
                                ...
                                if sl.covers_line_segment_tol(line_segment, epsilon, max_relative) {
                    ...
                    combined = match combined {
                        Some(c) => {
                            if relative_eq!(
                                ...
                                {
                                    if new_combined.covers_arc_segment_tol(
                                        arc_segment,
                                        epsilon,
                                        max_relative,
                                    ) {
                                ...
                                // Replace the old combined segment with the new line segment
                                if sa.covers_arc_segment_tol(arc_segment, epsilon, max_relative) {
                                    return Ok(Covered::OnBoundary(SegmentKey::from_segment_idx(
                                        idx,
                                    )));
                                }
...
fn intersections_between_polysegment_and_segment<'a, L>(
    left: L,
    right_idx: usize,
    right_seg: &'a Segment,
...
fn intersections_between_polysegment_and_segment_par<'a, L>(
    left: L,
    right_idx: usize,
    right_seg: &'a Segment,
    self_intersection: bool,
...
fn segment_intersections<'a>(
    ...
    if self_intersection {
        // Segments can't intersect themselves. To avoid evaluating segment
        // intersections twice (segment A = left_seg, segment B = right_seg
        // the same result as segment B = left_seg, segment A = right_seg), also
        // skip one of the pairs
        if left_idx >= right_idx {
            return None;
        }

        // Skip very small ("glue") segments to avoid false positives
        if relative_eq!(
            left_seg.start(),
            left_seg.stop(),
            epsilon = epsilon,
    ...
    return Some(
        left_seg
            .intersections_segment_tol(right_seg, epsilon, max_relative)
            .into_iter()
